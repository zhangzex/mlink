...
// Gateway implements gateway protocol. >Gateway实现网关协议。
type Gateway struct {
	protocol
	timer   *timingWheel
	servers [2]*link.Server

	physicalConnID uint32 //物理连接ID
	physicalConns  [connBuckets][2]*link.Channel //物理连接

	virtualConnID      uint32  // 虚拟连接ID
	virtualConns       [connBuckets]map[uint32][2]*link.Session //虚拟连接
	virtualConnMutexes [connBuckets]sync.RWMutex
}

// NewGateway create a gateway. >创建新的gateway
...
// ServeClients serve client connections. >ServeClients提供是客户端连接服务 
...
// ServeServers serve server connections. >ServeServers 提供服务端连接服务
...
func (gs *gwState) Dispose() {
	gs.disposeOnce.Do(func() {
		...
		// Close releated virtual connections >关闭相关的虚拟连接
